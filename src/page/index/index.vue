 ...
 * 减CD方法
 * @param rawCD 原CD
 * @param perCentArr 减CD百分比数组
 */
const reduceCD = (rawCD: number, perCentArr: number[]) => {
    let newCD = rawCD
    perCentArr.forEach((v) => {
        newCD = newCD - (newCD * (v / 100))
    })
    console.log(newCD + ' s')
    return newCD
}
...
[会话模式]: 保存上下文, 需要自行提供 OpenAI API 的 PrivateKey, 会消耗大量提示令牌(prompt_tokens), 到达最大值4096会请求失败且SessionId失效, 需刷新页面生成新Id。
...
const postAi = async () => {
    ...
    //     prompt: data.prompt,
    // });
    let res = await app.postAi(app.api.chatCompletionsAi, {
        sessionId: sessionId,
        apiKey: data.apiKey,
        prompt: data.prompt,
    });
    ...
    if (res.code == 200) {
        // let text = res.data['choices'][0]['text'] // text-davinci-003
        let text = res.data['choices'][0]['message']['content'] // gpt-3.5-turbo
                    ...
                    <div class="openAi-box-bottom">
                        <n-spin :show="data.loadingAi" style="width: 100%;">
                            <n-input-group>
                                ...
                                <n-button @click="completions" color="#3653F8" :style="{ width: '80px' }">
                                    ...
                                    <span style="margin-right: 0.25rem;">Send</span>
                                </n-button>
                            </n-input-group>
                        </n-spin>
                        <n-space style="margin-top: 5px;" :size="8">
                            ...
                            <n-button @click="data.historyShow = true, aiBoxScroll(`#scrollbarAi-2 .n-scrollbar-container`)"
                                        ...
                                        <div class="openAi-box-bottom">
                                            <n-space>
                                                <n-popconfirm v-model:show="data.delAllShow" @positive-click=""
                                                    ...
                                                    确定要清空历史消息吗？
                                                    <template #action>
                                                        <n-button @click="delAll(), data.delAllShow = false" type="error"
                                                            size="small">
                                                            确定
                                                        </n-button>
                                                    </template>
                                                ...
                                                <n-popconfirm v-model:show="data.delAllImgShow" @positive-click=""
                                                    ...
                                                    确定要删除所有图片消息及其提示吗？
                                                    <template #action>
                                                        <n-button @click="delImg(), data.delAllImgShow = false" type="error"
                                                            size="small">
                                                            确定
                                                        </n-button>
                                                    </template>
