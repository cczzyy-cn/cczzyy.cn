...
const data = reactive<{
    ...
    contextArray: [
        ...
        },
        {
            type: 'ai-code',
            codeType: 'java',
            text: `import { createApp } from "vue";import App from "./App.vue"; \nimport { createApp } from "vue";import App from "./App.vue";`,
        },
        {
            type: 'ai',
            text: '错误',
...
const handleStr = (str: string) => {
    str = str.replace(/^\s+|\s+$/g, ''); // 删除头尾多个空格
    str = str.replace(/^(\n)+|(\n)+$/g, ''); // 删除头尾多个\n
    return str;
}

// 处理文本代码混合字符串，代码片段被 [[ai-code-编程语言]] 和 [[/ai-code-编程语言]] 包裹
const handleCodeText = (text: string) => {
    text = handleStr(text)
    let typeReg = /\[\[ai\-code\-(.*?)\]\]/g;
    if (!text.match(typeReg)) {
        return false;
    }
    // 代码标记字符串 [[ai-code-php]],[[ai-code-go]]
    let typeStr = text.match(typeReg)?.join(',');
    let reg = /\[\[ai\-code\-([^\]]*)\]\]([\s\S]*?)\[\[\/ai\-code\-([^\]]*)\]\]/;
    // [ 文本, 'php', php代码, 'php', 文本,...]
    var textArray = text.split(reg);
    let status = 0;
    let codeType = '';
    textArray.forEach((item) => {
        if (!item) return;
        if (typeStr?.indexOf(item) == -1 && status != 1) {
            data.contextArray.push({
                type: 'ai',
                text: handleStr(item),
                codeType: codeType,
        ...
        // 遇到 item = 'php' , status = 1
        if (typeStr?.indexOf(item) != -1 && status == 0) {
            status = 1;
            codeType = item;
            return
        }
        ...
        if (typeStr?.indexOf(item) != -1 && status == 2) {
            status = 0
            return
        }
    })
...
const aiBoxScroll = () => {
    nextTick(() => {
        let scrollbarAi = document.querySelectorAll(`#scrollbarAi .n-scrollbar-container`)[0]
...
<template>
    <div id="index">
        <NDrawer v-model:show="data.openAiShow" :height="'100%'" :placement="'top'">
            <n-drawer-content :title="`OpenAI [sessionId: ${data.sessionId}] (Esc 收起)`" closable>
                    ...
                    <div class="openAi-box-content">
                        <n-scrollbar id="scrollbarAi" style="max-height: 100%">
                            <div v-for="item in data.contextArray">
                                ...
                                <!-- OpenAI Code -->
                                <div class="openAi-item" v-if="item.type == 'ai-code'">
                                    <div class="item-left">
                                        <div class="openAi-img">
                                            <svg width="20" height="20" viewBox="0 0 41 41" fill="none"
                                                xmlns="http://www.w3.org/2000/svg" stroke-width="1.5" class="w-6 h-6">
                                    ...
                                    <div class="item-right">
                                        <!-- <div v-html="newStr" style="display: flex;flex-direction: column;"></div> -->
                                        <highlightjs :language="item.codeType" :autodetect="false" :code="item.text">
                                        </highlightjs>
                                    </div>
                                </div>
                                <!-- 用户提问 -->
